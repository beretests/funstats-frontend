...
const StatCardBack: React.FC<StatCardBackProps> = ({ label }) => {
  ...
  const statData = getCardData(label) || [];
  ...
  return (
    <Card
      raised
      className={`absolute !bg-ok-800/90 w-full h-full shadow-lg hover:shadow-xl transition-shadow duration-300 !rounded-xl border-4 border-accent-100 backface-hidden`}
      style={{ transform: "rotateY(180deg)" }}
    >
      <CardContent className="flex flex-col items-center p-4">
        <div className={`text-ok-300 mb-2`}>
        ...
        <div className="relative flex-grow w-full flex items-center">
          {currentIndex > 0 && (
            ...
            >
              <ArrowBackIosIcon className="text-blue-700 dark:text-blue-300" />
            </IconButton>
          )}
          ...
          <div className="flex-grow text-center mx-14">
            {statData[currentIndex] && (
              <div key={currentIndex}>
                <h3 className="text-ok-300 font-fredoka">
                  {statData[currentIndex].away_team_name} v{" "}
                  {statData[currentIndex].home_team_name}
                </h3>
                <h4 className="text-center text-lg mb-2 text-blue-700 dark:text-blue-300 font-bold">
          ...
          {currentIndex < statData.length - 1 && (
            ...
            >
              <ArrowForwardIosIcon className="text-blue-700 dark:text-blue-300" />
            </IconButton>
          )}
        </div>
      </CardContent>
